...
public class DeOP {
    ...
    public static void deOP(MessageEvent me, IRCEvent e){
        String[] msgAr = me.getMessage().replaceFirst("`", "").split(" ");
        if(msgAr[0].equalsIgnoreCase("deop")){
            boolean notOp = true;
            ...
            if(msgAr.length >= 2){
                ...
                e.getSession().notice(me.getNick(), Colors.RED+"You need to include who you want to be deopped!");
            }
        }
    }
}
