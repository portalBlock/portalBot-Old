...
public class ImageMessage {
    ...
    public ImageMessage appendCenteredText(String... text) {
        for (int y = 0; y < lines.length; y++) {
            if (text.length > y) {
                int len = 65 - lines[y].length(); //Replaced ChatPaginator.AVERAGE_CHAT_PAGE_WIDTH with 65
    ...
    public void sendToUser(Session session, String nick) {
        for (String line : lines) {
            session.notice(nick, ChatTrans.translateColorCodes(line));
        }
    }
}
